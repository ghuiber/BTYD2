...
\section{Gabi's Disclaimer}

The BTYD2 package is my attempt to patch the original BTYD package using
a fix proposed \href{https://github.com/theofilos/BTYD}{here}. This
walkthrough still applies, so everywhere below this line where you see
"the BTYD package" you should use it interchangeably with "the BTYD2 
package." Everything below this line is the authors' original text, 
save for evaluated references to BTYD file paths, libraries, etc. 
which all now point to their BTYD2 counterparts.
